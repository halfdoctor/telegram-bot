...
// Try corrected ABI first, fall back to original
let ORCHESTRATOR_ABI;
try {
  ORCHESTRATOR_ABI = JSON.parse(fs.readFileSync(__dirname + '/deployments/Orchestrator-corrected.json', 'utf8')).abi;
  console.log('✅ Config.js using corrected Orchestrator ABI');
} catch (error) {
  console.log('⚠️ Config.js using original Orchestrator ABI (corrected not found)');
  ORCHESTRATOR_ABI = JSON.parse(fs.readFileSync(__dirname + '/deployments/Orchestrator.json', 'utf8')).abi;
}
